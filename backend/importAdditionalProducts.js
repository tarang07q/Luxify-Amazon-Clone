...
const additionalProducts = require('./data/additionalProducts');
...
const importAdditionalProducts = async () => {
  try {
    ...
    const productsWithUser = additionalProducts.map(product => ({
      ...product,
      user: admin._id
    }));

    console.log(`Preparing to import ${productsWithUser.length} additional products...`.yellow);
    ...
    console.log(`Additional products imported successfully!`.green.inverse);
    process.exit();
  } catch (err) {
