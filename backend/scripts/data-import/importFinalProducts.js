...
const finalProducts = require('./data/finalProducts');
...
const importFinalProducts = async () => {
  try {
    ...
    const productsWithUser = finalProducts.map(product => ({
      ...product,
      user: admin._id
    }));
    
    console.log(`Preparing to import ${productsWithUser.length} final products...`.yellow);
    ...
    console.log(`Final products imported successfully!`.green.inverse);
    process.exit();
  } catch (err) {
